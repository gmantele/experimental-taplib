...
public class TestConfigurableServiceConnection {
	...
	public static void setUp() throws Exception{
		...
		udfsWithClassNameProp.setProperty(KEY_UDFS, "[toto(a string)->VARCHAR, {tap.config.UDFToto}]");
		...
		udfsWithWrongParamLengthProp.setProperty(KEY_UDFS, "[toto(a string)->VARCHAR, {tap.config.UDFToto}, foo]");
	...
	public void testDefaultServiceConnectionProperties(){
		...
		try{
			...
			assertEquals(TAPException.class, e.getClass());
			assertEquals("Wrong UDF declaration syntax: only two items (signature and class name) can be given within brackets. (position in the property " + KEY_UDFS + ": 47)", e.getMessage());
		}
